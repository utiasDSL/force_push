...
def main():
    # TODO is this actually an important cut-off point?
    ...
    # state of tray
    P_tw_w = np.array(P_ew_w)
    R_we = util.rotation_matrix(P_ew_w[2])
    P_tw_w[:2] = np.array(P_ew_w[:2]) + R_we @ p_te_e
    V_tw_w = np.zeros(3)

    # full system state: joints + tray
    X = np.concatenate((q0, dq0, P_tw_w, V_tw_w))
    ...
    def ineq_constraints(X_ee, a_ee, p_te_e, jnp=jnp):
    ...
    def ineq_constraints_unrolled(X0, X_ee_d, var):
        # var is now just the lifted joint acceleration inputs
        X_q = X0[:ns_q]
        ineq_con = jnp.zeros(MPC_STEPS * nc_ineq)

        # P_tw_w = X0[ns_q:ns_q+3]
        # P_ew_w = model.ee_position(X0[:ni])
    ...
    for i in range(N - 1):
        ...
        V_tw_w = np.array([tray.body.velocity.x, tray.body.velocity.y, tray.body.angular_velocity])

        X = np.concatenate((X_q, P_tw_w, V_tw_w))
